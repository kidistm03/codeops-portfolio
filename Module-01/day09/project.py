...
print("\nTotal Transactions")
print(
    registry.total_transactions(101)
)
class Branch:
...
bahir = Branch("Bahir Dar Region")
bole = Branch("Bole Branch")
arada = Branch("Arada Branch")
head.add_child(addis)
head.add_child(bahir)
addis.add_child(bole)
addis.add_child(arada)
bole.add_account(account1)
arada.add_account(account2)
bahir.add_account(account3)
...
def bfs(transfers,start):
    ...
    while queue:
        ...
        if current not in visited:
            visited.add(current)
            for account in transfers[current]:
